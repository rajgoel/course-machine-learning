...
"""
function train_step!(q_network, target_network, optimizer, replay_buffer, batch_size, discount_factor, env_actions, target_evaluation)
...
- `target_evaluation=dqn_target_evaluation`: Function for target Q-value evaluation (use `ddqn_target_evaluation` for Double DQN)
...
```julia
env = BreakoutEnv()
q_network = DQN(env, target_evaluation=ddqn_target_evaluation)
```
...
function DQN(env; max_episodes=100_000, max_steps_per_episode=20_000, replay_memory_size=1_000_000, 
                                  update_frequency=4, target_update_frequency=25_000, batch_size=32, learning_rate=5e-5, discount_factor=0.99, 
                                  replay_start_size=100_000, initial_exploration=0.5, final_exploration=0.01,
                                  exploration_steps=100_000, hidden_layers=[128, 64], callback=EpisodeLogger(), target_evaluation=ddqn_target_evaluation)
    ...
    for episode in 1:max_episodes
        ...
        for step in 1:max_steps_per_episode
            ...
            if total_steps > replay_start_size && total_steps % update_frequency == 0
                loss = train_step!(q_network, target_network, optimizer, replay_buffer, batch_size, discount_factor, RL.actions(env), target_evaluation)
            end
