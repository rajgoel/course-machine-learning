...
Provides LeNet5 struct following the same API pattern as FluxDNN.
...
export LeNet5, train!, predict, accuracy, evaluate
...
```
"""
mutable struct LeNet5
    input_dims::Vector{Int}
    model::Flux.Chain
    layer_dimensions::Vector{Any}
    
    function LeNet5(input_dims::Vector{Int}; ϕ = Flux.relu)
...
"""
function predict(network::LeNet5, X::Array{Float32, 4})
...
"""
function accuracy(network::LeNet5, X::Array{Float32, 4}, Y_onehot)
...
"""
function train!(network::LeNet5, X_train::Array{Float32, 4}, Y_train, η, epochs; 
...
"""
function evaluate(network::LeNet5, X_test::Array{Float32, 4}, Y_test, num_classes)
