...
mutable struct DNN
    ...
    function DNN(layers::Vector{Int}; ϕ = Flux.relu)
...
# Returns
- `NamedTuple`: (train_losses=Vector{Float32}, val_losses=Vector{Float32})
"""
function train!(network::DNN, X_train::Matrix{Float32}, Y_train, X_validate::Matrix{Float32}, Y_validate,
               η, epochs; batch_size=128, verbose=true, early_stopping_patience=10)
...
"""
function predict(network::DNN, X::Matrix{Float32})
...
"""
function accuracy(network::DNN, X_test::Matrix{Float32}, Y_test)
    # Get predictions
...
# Returns
- `NamedTuple`: (accuracy=Float32, predictions=Vector, true_labels=Vector, confusion_matrix=Matrix{Int})
"""
function evaluate(network::DNN, X_test::Matrix{Float32}, Y_test, classes)
    # Get predictions
    ŷ = predict(network, X_test)
